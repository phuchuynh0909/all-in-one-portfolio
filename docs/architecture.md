...
Large-order blocks and trade-flow features are both aggregated inside ClickHouse
by materialized views on `ticks`, so neither needs a worker container nor a
reconciliation pass — `large_order_reconciler` is retired, and the
`block_episode_reconciler` / `large_execution` detector has been removed
entirely. Trade-flow anomaly scoring
(Isolation Forest) runs on demand in the backend at
