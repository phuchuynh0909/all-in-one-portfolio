"""Large-order ("Layer 3") daily block reconciler.

Same DNSE GraphQL API as the tick reconciler — only the symbol scope changes:
instead of the single front-month VN30F contract, this loops over every symbol
in the watchlist. For each symbol it pulls the day's full tape, merges trades
into fixed-second blocks per (symbol, side, bucket) — identical aggregation to
the live ingest — keeps the blocks whose total notional clears the threshold,
and upserts them into `large_orders`.

Because a block's size can grow as more fills arrive, the reconciler is
authoritative: it upserts *all* large blocks for the day (ReplacingMergeTree
overwrites any partial block the live stream emitted), and reports how many
keys were new.

Run:
    python workers/large_order_reconciler.py                 # respects schedule guard
    ...
    python workers/large_order_reconciler.py --from-date 2026-06-01   # --to-date defaults to today
"""
...
# Allow `python workers/large_order_reconciler.py` (script-by-path) as well as
# `python -m workers.large_order_reconciler` by ensuring the worker root is on
...
from model import LARGE_ORDERS_CLICKHOUSE_TABLE, LARGE_ORDERS_CREATE_TABLE_DDL
...
# Independent run-state file so this reconciler never clobbers the tick one.
STATE_FILE = (
    Path(__file__).parent.parent / "state_dir" / "large_order_reconciler_run_state.json"
)


# ---------------------------------------------------------------------------
# Schedule guard (mirrors reconciler_schedule, but on a separate state file)
# ---------------------------------------------------------------------------
...
def mark_run_done(date_str: str) -> None:
    ...
    log.info("Marked large-order reconciler run done for %s", date_str)
...
def _block_key(row: dict) -> tuple[str, str, int]:
    """Dedup key of a block: (symbol, bucket_start_iso, side)."""
    return (
        str(row["symbol"]),
        _to_utc(row["sending_time"]).isoformat(timespec="microseconds"),
        int(row["side"]),
    )
...
# Fetch / diff / upsert
# ---------------------------------------------------------------------------
def fetch_session_blocks(symbol: str, date_str: str, client: DNSEClient) -> list[dict]:
    """Fetch the day's tape for *symbol*, merge into in-session large blocks."""
    day = date.fromisoformat(date_str)
    session_start_utc, session_end_utc = _session_window_utc(date_str)
    window = config.large_order.window_seconds
    ...
    blocks = merge_ticks_into_blocks(in_session, window)
    return [b for b in blocks if is_large_block(b, min_value)]


def fetch_ch_block_keys(ch_client, db: str, symbol: str, date_str: str) -> set[tuple]:
    session_start_utc, session_end_utc = _session_window_utc(date_str)
    symbol_escaped = symbol.replace("'", "''")
    sql = f"""
    SELECT symbol, sending_time, side
    FROM {db}.{LARGE_ORDERS_CLICKHOUSE_TABLE} FINAL
    ...
    return {
        _block_key({"symbol": r[0], "sending_time": r[1], "side": r[2]})
        for r in result.result_rows
    }


def upsert_blocks(ch_client, db: str, blocks: list[dict], dry_run: bool = False) -> tuple[int, int]:
    """Insert all blocks (ReplacingMergeTree dedups/overwrites by key)."""
    if not blocks:
        return 0, 0
    ...
    for block in blocks:
        b = dict(block)
        b["received_at"] = now_utc
        insert_rows.append(to_block_tuple(b))

    if dry_run:
        log.info("Dry-run: would upsert %d blocks into %s.%s", len(insert_rows), db, LARGE_ORDERS_CLICKHOUSE_TABLE)
        return len(insert_rows), 0

    table = f"{db}.{LARGE_ORDERS_CLICKHOUSE_TABLE}"
    try:
        ...
        log.info("Upserted %d blocks into %s", len(insert_rows), table)
        return len(insert_rows), 0
    except Exception:
        log.exception("ClickHouse insert failed for %d blocks", len(insert_rows))
        return 0, len(insert_rows)


def _ensure_table(ch_client, db: str) -> None:
    ch_client.query(LARGE_ORDERS_CREATE_TABLE_DDL.format(database=db))


@dataclass
class LargeOrderReconcilerMetrics:
    run_date: str
    symbols: int = 0
    blocks: int = 0          # large blocks computed from the API tape
    new_blocks: int = 0      # blocks whose key was absent in ClickHouse
    upserted_rows: int = 0
...
def run_reconciler(
    ...
    dry_run: bool = False,
) -> LargeOrderReconcilerMetrics:
    started = time.monotonic()
    if symbols is None:
        symbols = load_symbols(config.large_order.watchlist_file)
    metrics = LargeOrderReconcilerMetrics(run_date=date_str, symbols=len(symbols))
    ...
    for idx, symbol in enumerate(symbols, 1):
        try:
            try:
                blocks = fetch_session_blocks(symbol, date_str, client)
            ...
            ch_keys = fetch_ch_block_keys(ch_client, db, symbol, date_str)
            new_count = sum(1 for b in blocks if _block_key(b) not in ch_keys)
            metrics.new_blocks += new_count

            upserted, failed = upsert_blocks(ch_client, db, blocks, dry_run=dry_run)
            if dry_run:
            ...
            log.info(
                "[%d/%d] %s — %d large blocks, %d new%s",
                idx, len(symbols), symbol, len(blocks), new_count,
...
def _print_metrics(m: LargeOrderReconcilerMetrics) -> None:
    print(
        f"\n{'=' * 56}\n"
        f"  Large-Order Block Reconciler  —  {m.run_date}\n"
        f"{'=' * 56}\n"
        f"  Symbols            : {m.symbols:>12,}\n"
        f"  Large blocks       : {m.blocks:>12,}\n"
        f"  New blocks         : {m.new_blocks:>12,}\n"
...
def weekdays_in_range(from_date: date, to_date: date) -> list[date]:
    ...
    no ticks (0 blocks), so they're harmless.
    """
    days: list[date] = []
    cur = from_date
    while cur <= to_date:
...
def run_range(from_date: date, to_date: date, symbols, dry_run: bool) -> None:
    ...
    grand = LargeOrderReconcilerMetrics(run_date=f"{from_date}..{to_date}")
    for d in days:
        m = run_reconciler(d.isoformat(), symbols=symbols, dry_run=dry_run)
        _print_metrics(m)
        grand.blocks += m.blocks
        grand.new_blocks += m.new_blocks
