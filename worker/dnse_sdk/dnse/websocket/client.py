...
#
# Upstream routes market data solely on the abbreviated ``T`` tag, and the
# if/elif chain in _dispatch_message has no else. A frame that arrives without
# a ``T`` therefore falls off the end and is discarded with no exception, no
# warning, not even a debug line — while connect, auth, subscribe and the
# heartbeat all keep reporting success. DNSE's published Trade-Extra payload is
# a bare market-data object with no ``T`` at all, so subscribing to tick_extra
# upstream yields a connection that looks perfectly healthy and delivers
# nothing.
#
# Two changes below fix that for every channel, not just Trade-Extra:
#   * _infer_msg_type recovers the type from the frame's shape, using the
#     client's own subscription set to break ties.
#   * _dispatch_message gained the missing else, so anything still unroutable
#     is logged once per distinct shape instead of vanishing.
#
# Re-vendoring the SDK reverts this. test_dnse_ws_input.py asserts the routing
# still works, so that test is what will tell you.
# --------------------------------------------------------------------------

# Frames that carry no ``T`` are identified by the raw fields present on the
# wire. Ordered, most specific first; the first signature whose fields are all
# present wins. Where a shape cannot separate two types (a closed OHLC bar is
# shaped exactly like a live one), every candidate is listed and the
# subscription set decides.
_MSG_TYPE_SIGNATURES = (
    (("te", "t"), ("symbol", "matchPrice")),
    (("q",), ("symbol", "bid")),
    (("f",), ("symbol", "foreignInvestorTypeCode")),
    (("e",), ("symbol", "expectedTradePrice")),
    (("sd",), ("symbol", "securityGroupId")),
    (("s",), ("tscProdGrpId", "tradingSessionId")),
    (("b", "bc"), ("symbol", "resolution", "close")),
    (("mi", "emi"), ("indexName", "valueIndexes")),
)

# Leading segment of a channel name -> the ``T`` its frames represent, e.g.
# "tick_extra.G1.json" -> "te". Splitting on "." keeps tick/tick_extra and
# ohlc/ohlc_closed from prefix-matching each other.
_CHANNEL_TO_MSG_TYPE = {
    "tick": "t",
    "tick_extra": "te",
    "top_price": "q",
    "foreign": "f",
    "expected_price": "e",
    "security_definition": "sd",
    "session": "s",
    "ohlc": "b",
    "ohlc_closed": "bc",
    "market_index": "mi",
    "estimated_market_index": "emi",
    "order": "eo",
    "position": "ep",
}
...
class TradingClient:
    ...
    def _subscribed_msg_types(self) -> set:
        """LOCAL PATCH: the ``T`` tags the channels we subscribed to produce."""
        types = set()
        for channel in self._subscriptions:
            msg_type = _CHANNEL_TO_MSG_TYPE.get(str(channel).split(".")[0])
            if msg_type:
                types.add(msg_type)
        return types

    def _infer_msg_type(self, data: Dict[str, Any]) -> Optional[str]:
        """LOCAL PATCH: recover a missing ``T`` from the frame's shape.

        Returns None when the frame matches no known signature, so the caller
        logs it rather than guessing a type and handing from_dict a payload it
        will silently fill with None.
        """
        subscribed = None
        for candidates, fields in _MSG_TYPE_SIGNATURES:
            if not all(data.get(field) is not None for field in fields):
                continue
            if len(candidates) == 1:
                return candidates[0]
            if subscribed is None:
                subscribed = self._subscribed_msg_types()
            narrowed = [c for c in candidates if c in subscribed]
            if len(narrowed) == 1:
                return narrowed[0]
            # Subscribed to both, or to neither: trade and trade_extra are the
            # one pair the payload itself can separate — only trade_extra
            # carries the aggressor side.
            if set(candidates) == {"te", "t"}:
                return "te" if data.get("side") is not None else "t"
            return candidates[0]
        return None

    def _log_unrouted(self, data: Dict[str, Any]) -> None:
        """LOCAL PATCH: surface a frame nothing claimed, once per shape.

        Upstream dropped these without a trace, which is what made a feed that
        delivers nothing indistinguishable from a quiet market.
        """
        key = str(data.get("action") or data.get("a") or sorted(data)[:4])
        if not hasattr(self, "_unrouted_seen"):
            self._unrouted_seen = set()
        if key in self._unrouted_seen:
            return
        self._unrouted_seen.add(key)
        logger.warning(f"Unrouted frame (no T, no matching shape): {str(data)[:400]}")

    async def _dispatch_message(self, data: Dict[str, Any]) -> None:
        action = data.get("action") or data.get("a")
        # LOCAL PATCH: fall back to shape when the frame carries no "T".
        msg_type = data.get("T")
        if msg_type is None and action is None:
        ...
        if action == "subscribed":
            ...
            # LOCAL PATCH: upstream ended the chain here, so anything unmatched
            # was discarded in silence.
            self._log_unrouted(data)
