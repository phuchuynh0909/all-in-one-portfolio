...
return this.labels[index] || "";
    ...
    def _plot_indicators():
        ...
        for i, value in enumerate(indicators):
            ...
            color_by = value._opts.get('color_by')
            color_above = value._opts.get('color_above') or lightness(BULL_COLOR, .6)
            color_below = value._opts.get('color_below') or lightness(BEAR_COLOR, .6)
            if color_by is not None:
                color_by = np.asarray(color_by)

            for j, arr in enumerate(value):
                color = next(colors)
                legend_label = next(legends) if legends is not None else legend_labels[j]
                ...
                if is_overlay:
                    ...
                    if is_histogram:
                        if color_by is not None:
                            if color_by.ndim == 2 and color_by.shape[0] == len(value):
                                band_arr = color_by[j]
                            else:
                                band_arr = color_by
                            color_col = f'{source_name}_color'
                            source.add(np.where(arr > band_arr, color_above, color_below), color_col)
                            r2 = fig.vbar('index', BAR_WIDTH, source_name, source=source,
                                 legend_label=legend_label, color=color_col)
                        else:
                    ...
                    if is_histogram:
                        if color_by is not None:
                            ...
                            r = fig.vbar('index', BAR_WIDTH, source_name, source=source,
                                     legend_label=LegendStr(legend_label), color=color_col)
                        else:
                            cm = linear_cmap(source_name, [lightness(BEAR_COLOR, .6), lightness(BULL_COLOR, .6)], low=0, high=0)
                            r = fig.vbar('index', BAR_WIDTH, source_name, source=source,
